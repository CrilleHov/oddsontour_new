...
import { Trophy, Settings, RefreshCw, TrendingUp, Zap } from "lucide-react"
...
export default function LeaderboardPage() {
  ...
  const { summary, chartData, parChartData, playersInChart, prognosis } = useMemo(() => {
    ...
    const playedDates = Array.from(new Set(rows.map((r) => r.tavling)))
    const playedComps = playedDates.length
    const remainingComps = Math.max(0, totalComps - playedComps)

    const prognosisData = summary.map((s) => {
      const avgPointsPerGame = s.antalComps > 0 ? s.totalPoang / s.antalComps : 0
      const estimatedRemainingPoints = avgPointsPerGame * remainingComps
      const estimatedFinalPoints = s.totalPoang + estimatedRemainingPoints

      return {
        spelare: s.spelare,
        current: s.totalPoang,
        final: estimatedFinalPoints,
      }
    })
    ...
    return { summary, chartData, parChartData, playersInChart, prognosis: prognosisData }
  }, [rows, competitions])
            ...
            <Card>
              ...
              <CardContent className="flex flex-col gap-4">
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  {prognosis.slice(0, 4).map((p, idx) => (
                    <div key={p.spelare} className="rounded-lg border border-border bg-secondary/30 p-4">
                      <div className="text-sm text-muted-foreground">#{idx + 1}</div>
                      <div className="mt-1 font-bold text-foreground">{p.spelare}</div>
